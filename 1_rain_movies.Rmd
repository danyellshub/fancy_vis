...
daily_temp_long <- daily_p %>%
  dplyr::select(date, station, mean_temp, min_temp, max_temp)%>%
  gather(.,
         key = "type",
         value= "temp",
         -station, -date)
...
p3 <- ggplot(daily_wide,
       aes(x=AFF, y=FNL, color=type,group=seq_along(FNL)))+
  geom_point() +
  geom_abline()+
  labs(x = 'Air Force Temperature (°C)', y= 'Fort Collins Temperature (°C)')+
  transition_reveal(date) +
  theme(legend.title=element_blank())
  
gganimate::animate(p3, nframes=50)
...
co1 <- counties(state="Colorado", cb=TRUE)

co_elev_clip <- crop(co_elev, co1)

co_reprj <- spTransform(co1, crs(co_elev))

basemap_2 <- tm_shape(co_elev_clip)+
  tm_raster(style='cont', 
            palette = 'Blacks',
            title = "Elevation (m)")+
  tm_shape(co_reprj)+
...
saveGIF({
  ...
  map2 <- basemap_2 + tm_shape(subset_sf)+
    tm_dots(title.size='Mean Temperature (°C)',
            col='mean_temp',
            size='mean_temp',
            scale=1.5,
            style='cont',
...
},movie.name = 'mean_temp_flash_all.gif',
  interval=0.12,
  ani.width=1000,
  ani.height=800)
